...
const DEMO_PRODUCTS: Product[] = [
  {
    id: 1,
    referenceNo: "245436546",
    producer: "Durga Dairy Farm",
    productName: "Durga Ghee",
    variant: "Pure cow ghee · 500 ml",
    batchNo: "DG-0426",
    imageUrl:
      "https://images.unsplash.com/photo-1631206753348-db44968fd440?q=80&w=600&auto=format&fit=crop",
  },
];
...
export const verifyOtp = createAsyncThunk(
  ...
  async (
    ...
    return (
      state.verification.product ??
      (await fetchProductByReference(
        payload.referenceNo
      ))
    );
  }
...
const verificationSlice = createSlice({
  ...
  reducers: {
    ...
    goToStep(
      state,
      action: PayloadAction<FlowStep>
    ) {
      state.step = action.payload;

      // For the POC:
      // when OTP is completed and we move to result,
      // load the demo product.
      if (action.payload === "result") {
        state.referenceNo = "245436546";
        state.product = DEMO_PRODUCTS[0];
        state.status = "succeeded";
        state.error = null;
      }
    },
